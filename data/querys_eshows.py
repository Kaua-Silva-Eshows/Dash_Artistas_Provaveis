...
FROM (
  ...
  (
    (CASE WHEN F.FAVORITE = '01' THEN 1000 ELSE 0 END) +  
    (COUNT(DISTINCT CASE WHEN P.FK_CONTRANTE = O.FK_CONTRATANTE THEN P.ID END) * 2) +  
    (AGG.NUM_SHOWS * 1.5) +
    (AGG.QTD_CASAS * 0.5) +
    ((1 / (1 + (DATEDIFF(CURRENT_DATE, AGG.ULTIMA_DATA) / 365))) * 55)
  ) AS PONTUACAO,
